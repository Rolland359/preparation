...
def principale(liste,dossier_autre_pour_main):
    ...
    if len(liste) == 2 :
        ...
        print("Le dossier recherché est : ",os.getcwd())
        ...
        principale(dossier_autre_pour_main)
...
if __name__=="__main__":
    ...
    principale(mot_clef,parent)
    while True:
        drapeau_continu = input("Voulez vous effectué un autre opération ? Oui/Non (o-n) :")
        if drapeau_continu == "o" or drapeau_continu =="O":
            os.system("cls")
            numero_dossier = input("Numéro du dossier : ")
            print(projet_dossier.append(numero_dossier))
            principale(projet_dossier,parent)
