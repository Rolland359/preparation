# coding : "UTF-8"
import os
from readMe.py import *

pivot = ["/1_Donnees_Brutes/By FMAP/","/2_Preparation/","/6_Livraison/1_Livraison_Production/"]

def recherche_donnee_finalisation(projet,bc, parametre, dossier="",date = "2024"):
    ...
    for elm in os.listdir():
        ...
        if projet.upper() in elm and bc.upper() in elm and "_2D" in elm:
            liste_obj.append(elm)
            # si le projet et le bc correspont on concatène avec le donnée brute
    if len(liste_obj)>1:
        print("plusieur projet correspond à votre critère")
        for elm in liste_obj:
            print(elm)
...
def main(liste,dossier_autre_pour_main):
    ...
    if len(liste) == 2 :
        ...
        recherche_donnee_finalisation(liste[0],liste[1],dossier_autre_pour_main)
    elif len(liste) == 3:
        # l'utilisateur a mis le dossier mais pas de date
        recherche_donnee_finalisation(liste[0],liste[1],dossier_autre_pour_main,liste[2])
    elif len(liste) == 4:
        # l'utilisateur a saisi la date et le dossier
        recherche_donnee_finalisation(liste[0],liste[1],dossier_autre_pour_main,liste[2],liste[3])
    else:
